function [x, k, rho] = gs(A, b, tol, kmax, x0)
...
%Creazione della matrice di Gauss-Seidel
Bgs = (D-E)\F; 

%Creazione del vettore f da usare per le iterazioni
f = (D-E)\b;

%Calcolo del raggio spettrale della matrice di Gauss-Seidel per controllare
%la convergenza
rho = max(abs(eig(Bgs))); 
...
while (norm(x_new-x_old)>tol*norm(x_old)) && (k<kmax)
    ...
    x_new = Bgs*x_old + f; % calcolo l'iterazione successiva
end
